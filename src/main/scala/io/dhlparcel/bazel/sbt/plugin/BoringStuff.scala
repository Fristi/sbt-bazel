...
object BoringStuff {
  ...
  private def parsePomFile(scalaVersion: ScalaVersion, log: Logger)(file: File): Option[Dependency] = {
    try {
      ...
      val artifactId = parseModuleName(scalaVersion)(rawName)

      Some(Dependency(organization, artifactId, version, None, None, None))
    } catch {
      case NonFatal(e) =>
  ...
  private def parseModuleName(scalaVersion: ScalaVersion)(rawName: String): ArtifactId = {
    if (rawName.endsWith(s"_${scalaVersion.binary}"))
      ArtifactId(rawName.replaceAllLiterally(s"_${scalaVersion.binary}", ""), Some(rawName))
    else if (rawName.endsWith(s"_${scalaVersion.full}"))
      ArtifactId(rawName.replaceAllLiterally(s"_${scalaVersion.full}", ""), Some(rawName))
    else
      ArtifactId(rawName, None)
  }
